...
inline std::vector<uint64_t> softnms(const std::vector<std::array<float, 4>>& bboxes,            //
...
)
{
    ...
    std::deque<uint64_t> sortedIndices = ::sortIndexes(temp_scores);
    ...
    for (int count=0;count<boxesLength;++count){
        ...
        for (const uint64_t elem : sortedIndices) {
            ...
            if (iou>= overlapThresh){
                float weight = 1-iou; //Linear weight 
                temp_scores[elem] *= weight;
            }
            if (temp_scores[elem]>=confThresh){
                newSortedIndices.emplace_back(elem);
            }
            else {
                temp_scores.erase(std::remove(temp_scores.begin(), temp_scores.end(), elem), temp_scores.end());
            }
        }
...
inline std::vector<uint64_t> classwise_nms(const std::vector<std::array<float, 4>>& bboxes,            //
                                 const std::vector<float>& scores,                           //
                                 const std::vector<uint64_t>& classes,                           //
                                 std::unordered_map<int, float> overlapThresh,
                                 const uint64_t topK = std::numeric_limits<uint64_t>::max()  //
)
{
    assert(bboxes.size() > 0);
...
// keep only topk bboxes for (uint64_t i = 0; i < boxesLength - realK; ++i) { sortedIndices.pop_front(); }
    ...
    while (!sortedIndices.empty()) {
        ...
        const auto& curBbox = bboxes[currentIdx];
        const float curArea = areas[currentIdx];
        const int curClass = classes[currentIdx];
