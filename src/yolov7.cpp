...
std::vector<float>  Yolov7::preprocess(std::string img_path, int height, int width , int channel , size_t batch_index)
  {
...
std::tuple<std::vector<std::array<float, 4>>, std::vector<float>, std::vector<uint64_t>> Yolov7::postprocess(const std::vector<std::vector<float>> &inferenceOutput,
                                                                                                             ...
                                                                                                             const int64_t batch_ind) const
{
  ...
  // std::cout<< "inferenceOutput size is " <<inferenceOutput.size() << std::endl;
  ...
  if ((inferenceOutput.size() > 1))
  {
    ...
    {
      std::cout << inferenceOutput[i].data()[0] << inferenceOutput[i].data()[1] << inferenceOutput[i].data()[2] << inferenceOutput[i].data()[3] << std::endl;
      std::cout << inferenceOutput[i].size() << std::endl;
                                      ...
                                      std::vector<uint64_t> &classIndices, const int b) const
{
  ...
  for (uint64_t i = 0; i < feature_map_size; ++i)
  {
    for (uint64_t j = 0; j < num_anchors; ++j)
    {
      ...
      std::cout << "printing box in 0 to 1 scale" << endl;
      std::cout << xt << " " << yt << " " << width << " " << height << std::endl;
